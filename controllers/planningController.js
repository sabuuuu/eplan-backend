    ...
    try {
        ...
        for (let i = 0; i < exams.length; i++) {
            const availableProfessors = profs.filter(prof => {
                ...
                const availability = prof.dispo.find(slot => slot.jour === exams[i].date);
                if (!availability) return false; // Professor is not available on the exam date
        
                // Check if any of the parsed time slots overlap with the professor's availability
                return parsedTimeSlots.some(timeSlot => {
                    const professorStart = moment(availability.debut, 'HH:mm');
                    const professorEnd = moment(availability.fin, 'HH:mm');
                    const timeSlotStart = moment(exams[i].date + ' ' + timeSlot.start, 'YYYY-MM-DD HH:mm');
                    const timeSlotEnd = moment(exams[i].date + ' ' + timeSlot.end, 'YYYY-MM-DD HH:mm');
                    return (
                        (timeSlotStart.isSameOrAfter(professorStart) && timeSlotStart.isBefore(professorEnd)) ||
                        (timeSlotEnd.isSameOrAfter(professorStart) && timeSlotEnd.isBefore(professorEnd))
                    );
                });
            });
        ...
        // Fetch available rooms from the request body
